...
export function resolve_tfolder(app: App, folder_str: string): TFolder {
    ...
    if (!folder) {
        throw new FileSorterError(`Folder "${folder_str}" doesn't exist`);
    }
    if (!(folder instanceof TFolder)) {
        throw new FileSorterError(`${folder_str} is a file, not a folder`);
    }
...
export function resolve_tfile(app: App, file_str: string): TFile {
    ...
    if (!file) {
        throw new FileSorterError(`File "${file_str}" doesn't exist`);
    }
    if (!(file instanceof TFile)) {
        throw new FileSorterError(`${file_str} is a folder, not a file`);
    }
